...
async def _main(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"    {'✓' if settings.api.odds_api_key else '○'} Vegas odds (The Odds API)    "
        f"{'active' if settings.api.odds_api_key else 'add ODDS_API_KEY'}\n"
        f"    {'✓' if settings.api.fred_api_key else '○'} FRED economic data           "
        f"{'active' if settings.api.fred_api_key else 'add FRED_API_KEY'}\n"
        f"    {'✓' if settings.api.newsapi_key else '○'} NewsAPI headlines            "
        f"{'active' if settings.api.newsapi_key else 'add NEWSAPI_KEY'}\n"
        f"    {'✓' if settings.api.metaculus_api_key else '○'} Metaculus predictions      "
        f"{'active' if settings.api.metaculus_api_key else 'add METACULUS_API_KEY'}\n"
        f"{'='*60}\n"
